...
class Translator {
  ...
  translate(text, dict, titles, timeRegex, locale) {
    ...
    // Search for title words and add them to matchesMap
    ...
    // Search for spaced words and add them to matchesMap
    ...
    // Search for individual words and add them to matchesMap
    lowerText.match(/(\w+([-'])(\w+)?['-]?(\w+))|\w+/g).forEach((word) => {
      if (dict[word]) matchesMap[word] = dict[word];
    });

    // Search for time words and add them to matchesMap
